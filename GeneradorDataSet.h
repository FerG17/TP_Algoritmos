...
string randomEmail(string nombre, string apellido, mt19937& gen) {
...
int randomDni(mt19937& gen, std::set<int>& usados) {
    int dni;
...
Cliente generarClienteAleatorio(mt19937& gen, std::set<int>& dnisUsados, vector<Compra>& comprasGeneradas) {
...
void generarDatasetClientes(int cantidadClientes, const string& archivoClientes, const string& archivoCompras) {
    ...
    std::set<int> dnisUsados; 
    vector<Compra> compras;

    for (int i = 0; i < cantidadClientes; ++i) {
        Cliente c = generarClienteAleatorio(gen, dnisUsados, compras);
